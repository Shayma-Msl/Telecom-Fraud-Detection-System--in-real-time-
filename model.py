...
class GAT_COBO(nn.Module):
    ...
    def forward(self, inputs):
        self.g = self.g.to(inputs.device)  # Ensure graph is on correct device
        logits_inter_GAT = self.transform_features(inputs)  # properly define this
        ...
        # ✅ Safety check
                # Flatten attention if it has extra dimensions
        if attention.dim() == 3:
            attention = attention.squeeze(-1)  # removes the last dim if it's 1
        if attention.dim() == 2 and attention.shape[1] > 1:
            attention = attention.mean(dim=1)  # average over heads if needed
        elif attention.dim() == 2 and attention.shape[1] == 1:
            attention = attention.squeeze(1)

        assert attention.dim() == 1, f"Attention must be 1D after processing, got {attention.shape}"


        return logits_inter_GAT, logits_inner_GAT, attention  # return embeddings too

 
## you are returning the embeddings, could be usedd for the classifier
